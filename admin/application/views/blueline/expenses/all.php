    ...
    <?php $month2 = $month != 0 ? $month : date('m'); $year =  $year ? $year : date("Y")?>
      <div class="col-md-3 col-xs-6 tile"><div class="icon-frame"><i class="ion-ios-calendar-outline hidden-xs"></i> </div><h1> <?php if(isset($expenses_this_month)){echo $expenses_this_month;} ?> <span class=""><?=$this->lang->line('application_expenses');?></span></h1><h2 class=""><?php echo $days_in_this_month == 12 ? $this->lang->line('application_in')." ".$year : $this->lang->line('application_in')." ".$this->lang->line('application_'.date("M", strtotime($year."-".$month2."-01"))); ?></h2></div>
      <div class="col-md-3 col-xs-6 tile"><div class="icon-frame secondary"><i class="ion-ios-cart hidden-xs"></i> </div><h1> <?php if($expenses_owed_this_month[0]->owed != ""){echo display_money(sprintf("%01.2f", round($expenses_owed_this_month[0]->owed, 2)));}else{ echo "0.00";} ?> </h1><h2 class=""><?=$this->lang->line('application_owed_in');?> <?php echo $days_in_this_month == 12 ? $year : $this->lang->line('application_'.date("M", strtotime($year."-".$month2."-01")));?></h2></div>
